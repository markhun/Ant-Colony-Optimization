...
class Enviroment:
    ...
    def __readData(self, file_name):
        ...
        problem_instance = pathlib.Path(__file__).parent.resolve()
        problem_instance = problem_instance / "../test_instances" / file_name

        jobs = []
        with open(problem_instance, "r") as file:
            for line in file:
                machines = {}
                this_machine = None
    ...
    def __buildGraph(self, init_pheromone):
        ...
        """
        num_nodes = (
            self.num_jobs * self.num_machines + 1
        )  # All machineXjobs pair plus the initial virtual node
        ...
        mapping = {
            i: nodename for i, nodename in enumerate(node_names)
        }  # mapping: {0:(-1,-1), 1:(0,0), ...}
                ...
                for to_job in range(self.num_jobs):
                    for to_machine in range(self.num_machines):
                        ...
                        if from_job == to_job and from_machine == to_machine:
                            ...
                            desirability = 1 / int(execution_time)
                            G[(from_job, from_machine)][(to_job, to_machine)][
                                "pheromone"
                            ] = init_pheromone
                            G[(from_job, from_machine)][(to_job, to_machine)][
                                "desirability"
    ...
    def updatePheromone(self, evaporation_rate, cycle_edge_contribution):
        ...
        for edge in self.G.edges:
            ...
            new_pheromone = cycle_edge_contribution[edge] + (
                evaporation_rate * old_pheromone
            )
    ...
    def calculateMakespanTime(self, path):
        ...
        machine_task_moments = (
            []
        )  # Represent all the tasks on their moments in wich machine
